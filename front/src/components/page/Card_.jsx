...
function Card(props) {
  ...
  useEffect(() => {
    // console.log("db", db);
        db = [...db, ...propData];
    // console.log("prop len", propData.length);
    for (let i = 0; i < propData.length; i++) {
      console.log(`${i} ${propData[i].name}`);
    }
    // console.log("data card", db, db.length);
    setCurrentIndex(db.length - 1);
    setIsFetchData(true);
    // console.log("fetchData", isFetchData);
  }, [propData]);
  ...
  const childRefs = Array.from({ length: db.length }, () => React.createRef());
